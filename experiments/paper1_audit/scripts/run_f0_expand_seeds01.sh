#!/usr/bin/env bash
# Phase 2B-1 F0 expand: seeds 0/1 after F0_GO on 2/3/4.
# Same recipe. Does not retrain 2/3/4. No F1. No Day5. No RX2. No retune.
# 5-seed clean FAIL skips stress on 0/1. Default: sequential on GPU 5.
set -euo pipefail
...
for seed in 2 3 4; do
  if [[ ! -f "${OUT_ROOT}/eval_val/${NAME}/seed_${seed}/metrics.json" ]]; then
    echo "missing frozen F0 seed ${seed}; run run_f0_identity_first.sh first" >&2
    exit 1
  fi
done
...
run_train() {
  ...
  fi
  {
    ...
    echo "pretrained=FORBIDDEN  f1=FORBIDDEN  day5=FORBIDDEN  rx2=FORBIDDEN"
...
out_json = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_5seed_stability.json")
out_md = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_5seed_stability.md")
SEEDS = [0, 1, 2, 3, 4]
...
payload = {
    ...
    "retrained_234": False,
    "recipe": "F0 identity-first 5-seed; 0/1 new; 2/3/4 frozen F0_GO",
    "seeds": SEEDS,
    "clean": {
        "rows": rows,
        "n_pass": f"{n_pass}/5",
        ...
        "pass": clean_ok,
        "gate": ">=4/5 Δ>=-2 pp, mean Δ>=-2, 0 collapse",
    },
    "scale": None,
    "full_rx": None,
    "verdict": "F0_5SEED_CLEAN_PASS" if clean_ok else "F0_5SEED_CLEAN_FAIL",
    "note": "Stress skipped on CLEAN_FAIL. Do not open RX2. Do not retune. Do not open F1.",
}
out_json.write_text(json.dumps(payload, indent=2) + "\n")
print("# F0 5-seed clean vs matching C'")
...
if not clean_ok:
    out_md.write_text(
        f"# F0 5-seed\n\n"
        f"verdict=F0_5SEED_CLEAN_FAIL  meanΔ={mean_d:+.2f}  pass={n_pass}/5  collapse={n_collapse}\n"
        "Do not interpret scale. Do not open RX2. Do not retune. Do not open F1.\n"
    )
    raise SystemExit(2)
PY
clean_status=$?
if [[ "${clean_status}" -eq 2 ]]; then
  echo "F0_5SEED_CLEAN_FAIL; stress skipped; RX2 closed"
  exit 0
fi
...
echo "======== F0 oob_scale stress seeds ${NEW_SEEDS[*]} (2/3/4 already frozen) ========"
run_wave run_stress "${NAME}_rx_oob_scale" --rx-factor oob_scale
echo "======== F0 full RX stress seeds ${NEW_SEEDS[*]} (2/3/4 already frozen) ========"
...
root = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/eval_val")
path = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_5seed_stability.json")
out_md = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_5seed_stability.md")
SEEDS = [0, 1, 2, 3, 4]
...
if payload["clean"]["pass"] and scale_verdict != "SCALE_FAIL":
    overall = "F0_5SEED_GO"
else:
    overall = "F0_5SEED_HOLD"
payload["scale"] = {
    "rows": scale_rows,
    "mean_pp": round(mean_s, 2),
    "std_pp": round(std_s, 2),
    "verdict": scale_verdict,
    "gate": "mean D<8 STRONG / all <15 and mean<15 PASS / else FAIL",
}
payload["full_rx"] = {
    "rows": full_rows,
...
payload["note"] = "RX2 still closed. Next Human GO only if F0_5SEED_GO. Do not retune. Do not open F1."
...
echo "F0 expand seeds 0/1 finished. Seeds 2/3/4 not retrained. Day5 unused. RX2 unused."
