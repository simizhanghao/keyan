#!/usr/bin/env bash
# 2B-0 Gate 2: S0 and S1 seeds 0/1, eval-only oob_scale.
# No training. No full RX (Gate 3 waits for scale PASS). Day5 unused. No RX2.
# Does not overwrite C_full_ratio_rx_* / C_fft / C1 stress dirs.
set -euo pipefail
...
run_eval() {
  ...
  {
    echo "=== EVAL ${eval_name} seed=${seed} GPU=${gpu} factor=oob_scale fft_source=full oob_norm=ratio Day4-only ==="
...
print(f"${eval_name} seed=${seed} file_acc={100*m['file_acc']:.1f}% window_acc={100*m['window_acc']:.1f}% files={m['num_files']} rx={m.get('rx_style_eval')} factor={m.get('rx_factor')} fft={m.get('fft_source')} oob={m.get('oob_norm')}")
...
if m.get("rx_factor") != "oob_scale":
    raise SystemExit(f"rx_factor must be oob_scale, got {m.get('rx_factor')}")
...
echo "oob_norm=ratio"
echo "rx_factor=oob_scale"
echo "day5_eval=FORBIDDEN"
echo "rx2=FORBIDDEN"
echo "full_rx=FORBIDDEN"
echo "training=FORBIDDEN"

run_arm "C_full_ratio_paired_clean" "C_full_ratio_paired_clean_rx_oob_scale"
run_arm "C_full_ratio_paired_scale" "C_full_ratio_paired_scale_rx_oob_scale"
...
print("# Gate 2 — oob_scale vs own clean (Day4 window; headline = S1)")
print("| arm | seed | clean | oob_scale | Δ | C' Δ | bin |")
print("| --- | ---: | ---: | ---: | ---: | ---: | --- |")
rows = []
for arm, clean_name, stress_name in (
    ("S0", "C_full_ratio_paired_clean", "C_full_ratio_paired_clean_rx_oob_scale"),
    ("S1", "C_full_ratio_paired_scale", "C_full_ratio_paired_scale_rx_oob_scale"),
):
    for seed in (0, 1):
        ...
        if clean.get("fft_source") != "full" or stress.get("fft_source") != "full":
            raise SystemExit("fft_source must stay full")
        if clean.get("oob_norm") != "ratio" or stress.get("oob_norm") != "ratio":
            raise SystemExit("oob_norm must stay ratio")
        drop = win(clean) - win(stress)
        row = {
            "arm": arm,
            "seed": seed,
            "clean_window": round(win(clean), 1),
            "oob_scale_window": round(win(stress), 1),
            "oob_scale_drop_pp": round(drop, 1),
            "oob_scale_file_drop_pp": round(fil(clean) - fil(stress), 1),
            "cprime_oob_scale_drop_pp": round(win(cprime) - win(cprime_scale), 1),
            "scale_reading": scale_bin(drop),
        }
        rows.append(row)
        print(
            f"| {arm} | {seed} | {row['clean_window']:.1f} | {row['oob_scale_window']:.1f} | "
            f"{row['oob_scale_drop_pp']:.1f} | {row['cprime_oob_scale_drop_pp']:.1f} | {row['scale_reading']} |"
        )

s0 = [r for r in rows if r["arm"] == "S0"]
s1 = [r for r in rows if r["arm"] == "S1"]
mean_s0 = sum(r["oob_scale_drop_pp"] for r in s0) / len(s0)
mean_s1 = sum(r["oob_scale_drop_pp"] for r in s1) / len(s1)
s1_bins = {r["scale_reading"] for r in s1}
if "FAIL" in s1_bins:
    headline = "FAIL"
else:
    headline = scale_bin(mean_s1)
...
out = {
    ...
    "rx_factor": "oob_scale",
    "rows": rows,
    "mean_s0_oob_scale_drop_pp": round(mean_s0, 1),
    "mean_s1_oob_scale_drop_pp": round(mean_s1, 1),
    "scale_reading": headline,
    "s1_seed_bins": sorted(s1_bins),
    "frozen_cprime_mean_oob_scale_drop_pp": 28.7,
}
path = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/s0_s1_rx_oob_scale.json")
path.write_text(json.dumps(out, indent=2) + "\n")
print(f"mean S0 drop {out['mean_s0_oob_scale_drop_pp']:.1f}  mean S1 drop {out['mean_s1_oob_scale_drop_pp']:.1f}")
print("scale_reading", headline)
print("wrote", path)
if headline == "FAIL":
    print("Gate 2 FAIL; do not retune; do not open full RX / RX2 / 5-seed")
else:
    print("Gate 2", headline, "; full RX is a later eval")
PY

echo "2B-0 Gate 2 oob_scale finished. Day5 unused. Full RX unused."
