#!/usr/bin/env bash
# Phase 2B-1 F0: identity-first paired-scale on CLEAN_FAIL seeds 2/3/4.
# Unique legal delta vs S1: --init-checkpoint = matching-seed C' full best.pt
# Frozen recipe. No --pretrained. No F1. No Day5. No RX2. No retune.
# Gate A FAIL skips stress. Default: sequential on GPU 5.
set -euo pipefail
...
for seed in "${SEEDS[@]}"; do
  if [[ ! -f "${OUT_ROOT}/runs/${CPRIME}/seed_${seed}/best.pt" ]]; then
    echo "missing C' ckpt seed ${seed}" >&2
    exit 1
  fi
  if [[ ! -f "${OUT_ROOT}/eval_val/${CPRIME}/seed_${seed}/metrics.json" ]]; then
    echo "missing frozen C' eval seed ${seed}" >&2
    exit 1
  fi
done
...
run_train() {
  local gpu="$1"
  local seed="$2"
  local init_ckpt="${OUT_ROOT}/runs/${CPRIME}/seed_${seed}/best.pt"
  ...
  {
    echo "=== TRAIN ${NAME} seed=${seed} GPU=${gpu} init=${init_ckpt} paired_view=oob_scale ==="
    echo "pretrained=FORBIDDEN  day5=FORBIDDEN  rx2=FORBIDDEN"
...
run_wave() {
  ...
  while [[ ${i} -lt ${#SEEDS[@]} ]]; do
    ...
    local status=0
    local j=0
    while [[ ${j} -lt ${#GPU_ARR[@]} && $((i + j)) -lt ${#SEEDS[@]} ]]; do
      local seed="${SEEDS[$((i + j))]}"
      local gpu="${GPU_ARR[${j}]}"
      "${fn}" "${gpu}" "${seed}" "${extra[@]}" &
      pids+=("$!")
      j=$((j + 1))
...
echo "python=${PY}"
echo "gpus=${GPUS}"
echo "train_seeds=${SEEDS[*]}"
echo "name=${NAME}"
echo "pretrained=FORBIDDEN"
echo "f1=FORBIDDEN"
echo "day5=FORBIDDEN"
echo "rx2=FORBIDDEN"
...
out_json = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_identity_first_gate.json")
out_md = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_identity_first_gate.md")
SEEDS = [2, 3, 4]
...
for seed in SEEDS:
    ...
    f0 = load("C_full_ratio_init_paired_scale", seed)
...
clean_ok = (n_pass == 3) and (n_collapse == 0)
payload = {
    "day5_used": False,
    "rx2_used": False,
    "f1_opened": False,
    "pretrained_used": False,
    "recipe": "F0 identity-first: init-checkpoint C' + paired oob_scale; 80/3e-3",
    "seeds": SEEDS,
    "clean": {
        "rows": rows,
        "n_pass": f"{n_pass}/3",
        ...
        "pass": clean_ok,
        "gate": "Gate A: 3/3 Δ>=-2 pp and no collapse",
    },
    "scale": None,
    "full_rx": None,
    "verdict": "GATE_A_PASS" if clean_ok else "GATE_A_FAIL",
    "note": (
        "Stress skipped on GATE_A_FAIL. Do not open F1 from stress. "
        "Do not retune a/lr/epoch. RX2 closed. F1 only if clean FAIL."
    ),
}
out_json.write_text(json.dumps(payload, indent=2) + "\n")
print("# F0 identity-first Gate A (clean vs matching C')")
print("| seed | C' win | F0 win | Δ | gate | collapse |")
print("| ---: | ---: | ---: | ---: | --- | --- |")
for r in rows:
    print(
        f"| {r['seed']} | {r['cprime_window']:.1f} | {r['f0_window']:.1f} | "
        f"{r['delta_clean_pp']:+.1f} | {'PASS' if r['clean_pass'] else 'FAIL'} | {r['collapse']} |"
    )
print(f"mean Δ={mean_d:+.2f}  pass={n_pass}/3  collapse={n_collapse}  verdict={payload['verdict']}")
print("wrote", out_json)
if not clean_ok:
    out_md.write_text(
        f"# F0 identity-first Gate A\n\n"
        f"verdict=GATE_A_FAIL  meanΔ={mean_d:+.2f}  pass={n_pass}/3  collapse={n_collapse}\n"
        "Do not interpret scale. Do not open RX2. Do not retune.\n"
        "Next authorized beat: F1 identity-anchored (same seeds/gates).\n"
    )
    raise SystemExit(2)
PY
clean_status=$?
if [[ "${clean_status}" -eq 2 ]]; then
  echo "GATE_A_FAIL; stress skipped; F1 not auto-opened; RX2 closed"
  exit 0
fi
...
echo "======== F0 oob_scale stress seeds ${SEEDS[*]} ========"
run_wave run_stress "${NAME}_rx_oob_scale" --rx-factor oob_scale
echo "======== F0 full RX stress seeds ${SEEDS[*]} ========"
run_wave run_stress "${NAME}_rx_style"
...
root = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/eval_val")
path = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_identity_first_gate.json")
out_md = Path("/data1/hcc/llm4RF/new_phase/experiments/paper1_audit/results/matched_seed0/f0_identity_first_gate.md")
SEEDS = [2, 3, 4]
...
full_rows = []
for seed in SEEDS:
    clean = load("C_full_ratio_init_paired_scale", seed)
    sc = load("C_full_ratio_init_paired_scale_rx_oob_scale", seed)
    fu = load("C_full_ratio_init_paired_scale_rx_style", seed)
...
std_s = statistics.stdev(scale_ds) if len(scale_ds) > 1 else 0.0
mean_f = statistics.mean(full_ds)
std_f = statistics.stdev(full_ds) if len(full_ds) > 1 else 0.0
all_lt15 = all(d < 15 for d in scale_ds)
if any(d >= 15 for d in scale_ds) or mean_s >= 15:
    scale_verdict = "GATE_B_FAIL"
elif mean_s < 8 and all_lt15:
    scale_verdict = "GATE_B_STRONG"
else:
    scale_verdict = "GATE_B_PASS"
full_verdict = "TRACKS_SCALE" if mean_f < 15 else ("WITHIN_IDEAL" if mean_f < 20 else "RESIDUAL")
gate_c_ok = mean_f < 15
if payload["clean"]["pass"] and scale_verdict != "GATE_B_FAIL":
    overall = "F0_GO"
elif payload["clean"]["pass"]:
    overall = "F0_HOLD_SCALE"
else:
    overall = "F0_CLEAN_FAIL"
payload["scale"] = {
    "rows": scale_rows,
...
payload["full_rx"] = {
    ...
    "gate_c_tracks": gate_c_ok,
    "hard_gate": False,
}
payload["verdict"] = overall
payload["note"] = (
    "RX2 still closed. Do not retune from this table. "
    "F0_GO → later expand to seeds 0/1. GATE_B_FAIL or clean fail → F1, not retune."
)
...
lines = [
    ...
    "",
    f"verdict={overall}  gate_a={payload['clean']['pass']}  gate_b={scale_verdict}  gate_c={full_verdict}",
...
echo "F0 seeds 2/3/4 finished. Day5 unused. RX2 unused. F1 not opened."
