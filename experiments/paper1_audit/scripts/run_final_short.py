...
def main():
 ...
 w.close();a.out.write_text(json.dumps({'protocol':{'model':a.model,'seed':a.seed,'epochs':a.epochs,'batch_size':64,'receivers':14,'validation':'none','blind_opened':False},'parameters':sum(p.numel() for p in m.parameters()),'history':h,'tensorboard':str((a.out.with_suffix('')/'tensorboard').resolve())},indent=2)+'\n')
