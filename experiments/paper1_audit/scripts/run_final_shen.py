...
def main():
    ...
    paths=sorted(a.data_root.glob('*_train.h5')); rx_map={q.stem:i for i,q in enumerate(paths)}; cache_root=a.data_root.parent/'cis_cache_20sdr'; train=CachedCISSet(paths,cache_root,rx_map); loader=DataLoader(train,64,shuffle=True,num_workers=4,pin_memory=True,persistent_workers=True,prefetch_factor=4)
