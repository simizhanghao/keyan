#!/usr/bin/env bash
# Generate download list / fetch OSU LoRa Diff_Configurations_Setup IQ files.
# Path template (verified against official index):
#   Diff_Configurations_Setup/Config{c}/IQ_{raw_device}.dat
#   Diff_Configurations_Setup/Config{c}/IQ_{raw_device}.sigmf-meta
...
SETUP_DIR="Diff_Configurations_Setup"
BASE_URL="https://research.engr.oregonstate.edu/hamdaoui/RFFP-dataset/LoRa-Dataset/${SETUP_DIR}"
LIST="${DATA_ROOT}/download_configs.tsv"
LOG="${ROOT}/logs/download_osu_lora_configs.out"
...
for config in (1, 2, 3, 4):
    for raw_device in range(1, 26):
        rel = f"Diff_Configurations_Setup/Config{config}"
        for name in (f"IQ_{raw_device}.dat", f"IQ_{raw_device}.sigmf-meta"):
            url = f"{base_url}/Config{config}/{name}"
...
{
  echo "=== download_osu_lora_configs.sh ==="
...

{
  ...
  for config in 1 2 3 4; do
    count=$(find "${DATA_ROOT}/${SETUP_DIR}/Config${config}" -name 'IQ_*.dat' 2>/dev/null | wc -l)
    echo "Config${config}: ${count}/25 IQ_*.dat files on disk"
  done
} | tee -a "${LOG}"
