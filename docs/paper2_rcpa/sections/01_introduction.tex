...
Prior work on the public OSU LoRa corpus reports that strict source-only cross-receiver transfer can remain near chance despite strong same-receiver performance on related submitted hybrid models~\cite{paper1_placeholder,elmaghbub2021lora_wild,shen2021lora_jsac}.
...
In our related submitted work, OOB evidence is fused with in-band representations through cross-attention on a fixed receiver~\cite{paper1_placeholder}.
...
We use a frozen OOB-guided RF-HSTU hybrid model from our related submitted work only as a feature extractor~\cite{paper1_placeholder}; \textbf{we do not claim a new backbone architecture or a published prior method}.
